...
def index_o_a(sxx, t_0, min_db):
    ...
    tau = list(range(-(t_0 - t_s), 0)) + list(range(1, t_0 - t_e + 1))

    ret_omega = zip(tau, r_t_omegas[::-1] + f_t_omegas)
    ret_alpha = zip(tau, r_t_alphas[::-1] + f_t_alphas)
...
def harma(audio_file, nsyllables):
    f, t, sxx = get_spectrogram(audio_file)
...
def get_spectrogram(audio_file):
    sample_rate, data_array = wav.read(audio_file)
    return signal.spectrogram(data_array, sample_rate, ('kaiser', 0.25), nperseg=1024, noverlap=768)
